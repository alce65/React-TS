...
import { useNotesFlux } from './use.notes2';
...
describe('Given custom hook useNotes', () => {
    ...
    function TestComponent(): JSX.Element {
        const { notes, loadState, loadNotes, add, update, erase } = useNotesFlux();
        return (
            <>
    ...
    describe('When the component run the hook with errors', () => {
        ...
        beforeEach(() => {
            (getNotes as jest.Mock).mockRejectedValueOnce(
        ...
        test('If we click button 1 error should send to console', async () => {
            const buttons = screen.getAllByRole('button');
            await userEvent.click(buttons[0]);
            expect(console.error).toHaveBeenCalledWith('Get All Error');
        });
    });
});
